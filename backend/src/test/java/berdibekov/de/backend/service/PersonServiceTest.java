...
class PersonServiceTest {
    ...
    @Test
    void deletePerson_shouldReturnNotFoundException_whenPersonDoesNotExist() {
        // when
        when(personRepository.findById(personId)).thenReturn(Optional.empty());

        Exception exception = assertThrows(NotFoundException.class, () -> personService.deletePerson(personId));

        String expectedMessage = String.format(NOT_FOUND_PERSON, personId);
    ...
    @Test
    void updatePerson_shouldReturnNotFoundException_whenPersonDoesNotExist() {
        // when
        when(personRepository.findById(personId)).thenReturn(Optional.empty());

        Exception exception = assertThrows(NotFoundException.class, () -> personService.updatePerson(person, personId));
    ...
    @Test
    void getOne_shouldReturnNotFoundException_whenPersonDoesNotExist() {
        // when
        when(personRepository.findById(personId)).thenReturn(Optional.empty());

        Exception exception = assertThrows(NotFoundException.class, () -> personService.getOne(personId));
